...
let
  ...
  configFile = (pkgs.formats.json { }).generate "nixbot-config.json" settings.config;
  listen = settings.listen or "/run/${name}/web.sock";
  overUnixSocket = !lib.isInt listen;
  user = settings.user or "nixbot";
  gcrootsDir = settings.config.gcroots_dir or "/nix/var/nix/gcroots/per-user/${user}";
  outputsPath = settings.config.outputs_path or null;
...
flakeletLib.mkService {
  ...
  services.${name} = {
    ...
    requires = [ "${name}.socket" ] ++ settings.requires or [ ];
    ...
    serviceConfig = {
      ...
      LoadCredential = lib.mapAttrsToList (id: path: "${id}:${path}") (settings.credentials or { });
  ...
  exports =
    lib.optionalAttrs (settings ? domain) {
      http.web = flakeletLib.contracts.http {
    ...
    // lib.optionalAttrs (lib.hasInfix "host=/run/postgresql" (settings.config.db_url or "")) {
