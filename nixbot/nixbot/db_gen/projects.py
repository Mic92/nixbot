...
async def upsert_discovered_projects(
    conn: ConnectionLike,
    *,
    forges: collections.abc.Sequence[str],
    forge_repo_ids: collections.abc.Sequence[str],
    owners: collections.abc.Sequence[str],
    names: collections.abc.Sequence[str],
    default_branches: collections.abc.Sequence[str],
    urls: collections.abc.Sequence[str],
    privates: collections.abc.Sequence[bool],
    enableds: collections.abc.Sequence[bool],
) -> None:
...
def project_visibility_rows(conn: ConnectionLike) -> QueryResults[ProjectVisibilityRowsRow]:
    def _decode_hook(row: asyncpg.Record) -> ProjectVisibilityRowsRow:
        return ProjectVisibilityRowsRow(id_=row[0], forge=row[1], forge_repo_id=row[2], owner=row[3], name=row[4], private=row[5])

    return QueryResults(conn, PROJECT_VISIBILITY_ROWS, _decode_hook)
...
def project_forge_ids(conn: ConnectionLike) -> QueryResults[ProjectForgeIdsRow]:
    def _decode_hook(row: asyncpg.Record) -> ProjectForgeIdsRow:
        return ProjectForgeIdsRow(id_=row[0], forge=row[1], forge_repo_id=row[2])

    return QueryResults(conn, PROJECT_FORGE_IDS, _decode_hook)
